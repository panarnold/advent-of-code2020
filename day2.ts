const input = `1-9 x: xwjgxtmrzxzmkx
4-6 r: rrrkrgr
4-5 v: vvfvvvn
...
//second
function solutioon(input: string) {
  return input.split("\n").reduce((validPass, currentPass): number => {
    const firstSplit = currentPass.split(": ");
    const firstPart: string = firstSplit[0];
    const password: string = firstSplit[1];

    const [occurencies, keyLetter] = firstPart.split(" ");
    const bounds: string[] = occurencies.split("-");
    const firstPosition = parseInt(bounds[0]);
    const secondPosition = parseInt(bounds[1]);

    const isAtFirstPosition: boolean =
      password[firstPosition - 1] === keyLetter;
    const isAtSecondPosition: boolean =
...
console.log(solutioon(input));

//part one

function polution(input: string) {
  return input.split("\n").reduce((validPass, currentPass): number => {
    const firstSplit = currentPass.split(": ");
    const firstPart = firstSplit[0];
    const password: string[] = firstSplit[1].split("");
    ...
    let howManyTimes: number = password.filter((l) => l === keyLetter).length;

    if (howManyTimes >= lowerBound && howManyTimes <= upperBound) {
      return validPass + 1;
    }
    return validPass;
  }, 0);
