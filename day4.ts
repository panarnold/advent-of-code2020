...
function isValid(validators: Array<Validator>, value: string): boolean {
  return reduce(
    map(validators, (validator) => validator(value)),
    (results, nextResult) => results && nextResult
  );
}

const ALLOWED_HAIRCOLOR_CHARS: string[] = [
  "a",
  "b",
  "c",
  "d",
...
const VALIDATORS_HCL = [
  ...
  (value: string): boolean =>
    reduce(
      map(drop(value, 1), (char): boolean =>
        includes(ALLOWED_HAIRCOLOR_CHARS, char)
      ),
      (prevResult, nextResult) => prevResult && nextResult
    ),
];
...
const VALIDATORS_PID = [
  ...
  (value: string): boolean =>
    reduce(
      map(
        value.split(""),
        (char) => isNumber(parseInt(char)) && isBetween(char, 0, 9)
      ),
...
const VALIDATORS_HGT = [
  (value: string): boolean =>
    (endsWith(value, "cm") && isBetween(value.slice(0, -2), 150, 193)) ||
    (endsWith(value, "in") && isBetween(value.slice(0, -2), 76, 193)),
];
...
const passports = input
  ...
  .map((passport) => fromPairs(map(passport, (field) => field.split(":"))))
  .filter((passport) =>
    reduce(
      map(requiredFields, (field) => has(passport, field.name)),
      (prev, next) => prev && next
    )
  ).length;

console.log("Number of valid passwords from part one: ", passports);
...
const passports2 = input
  ...
  .map((passport) =>
    reduce(
      map(requiredFields, (field) =>
        isValid(field.validators, passport[field.name])
      ),
      (results, result) => results && result
    )
  )
...
console.log("Number of valid passwords from part two: ", passports2);
