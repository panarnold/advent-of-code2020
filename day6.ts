...
const partOne = input
  ...
  .map((answerGroup) => uniq(answerGroup).length)
  .reduce((sum, prev) => prev + sum, 0);
...
let formList = input
  ...
  .map((formList) => [
    uniq(flatten(formList.map((string) => string.split("")))),
    formList,
  ])
  .map((form) => {
    let chars: string[] = form[0];
    let forms: string[] = form[1];
    let count = 0;

    forEach(chars, (char) => {
      if (every(forms, (form) => includes(form, char))) {
        count++;
      }
    });
    return count;
